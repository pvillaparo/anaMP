...
for i in range(len(df3)):
    if(df3["distancia"][i][0]=="["): #Busca en todo el archivo aquellos registros que empiecen con [ ya que nos indican que es la lista con coordenadas
        ...
        if((12-int(datetime.now().strftime("%H")))<0): #Si la hora actual es mayor que la hora a la que se desea salir
          td=(24+(12-int(datetime.now().strftime("%H"))))*60
          nuevahora=(datetime.now()+timedelta(hours=12-int(datetime.now().strftime("%H")))).strftime("%H:%M:%S")
        elif((12-int(datetime.now().strftime("%H")))>0):#Si la hora actual es menor que la hora a la que se desea salir
          td=(12-int(datetime.now().strftime("%H")))*60
          nuevahora=(datetime.now()+timedelta(hours=12-int(datetime.now().strftime("%H")))).strftime("%H:%M:%S")
        elif((12-int(datetime.now().strftime("%H")))==0):#Si la hora de salida es la misma a la actual
        ...
        try:#Probara calcular la ruta
          ...
          df3["tiempo"][i]=round(route_time,2) 
            ...
            print(e)
    print(i)
df3.to_csv("Calctiemposcorregido1.csv") #archivo de salida con las correcciones
